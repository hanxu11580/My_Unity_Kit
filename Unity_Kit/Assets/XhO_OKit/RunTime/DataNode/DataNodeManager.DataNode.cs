    ...
    public sealed partial class DataNodeManager : ManagerBase, IDataNodeManager
    {
        ...
        private sealed class DataNode : IDataNode, IReference
        {
            ...
            public static DataNode Create(string name, DataNode parent)
            {
                if (!IsValidName(name))
                {
                    throw new XhO_OKitException("Name of data node is invalid.");
                }
            ...
            public string FullName => m_Parent == null
                ? m_Name
                //有父节点 使用 . 连接
                : StringExtension.Format("{0}{1}{2}", m_Parent.FullName, PathSplitSeparator[0], m_Name);
            ...
            public bool HasChild(string name)
            {
                if (!IsValidName(name))
                {
                    throw new XhO_OKitException("Name is invalid.");
                }
            ...
            public void GetAllChild(List<IDataNode> results)
            {
                if (results == null)
                {
                    throw new XhO_OKitException("Results is invalid.");
                }
            ...
            {
                return StringExtension.Format("{0}: {1}", FullName, ToDataString());
            }
            ...
            public string ToDataString()
            {
                ...
                return StringExtension.Format("[{0}] {1}", m_Data.Type.Name, m_Data.ToString());
            }
